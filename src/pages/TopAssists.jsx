        ...
        <div className="space-y-12 animate-fadeIn pb-20 px-4 md:px-0">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/10 pb-8">
                <div className="text-center md:text-left space-y-2">
                    <p className="text-cyan-400 font-black uppercase tracking-[0.3em] text-[10px] md:text-xs">Playmaker Award</p>
                    <h1 className="text-3xl md:text-5xl font-black text-white italic tracking-tighter uppercase leading-none">TOP ASSISTS</h1>
                </div>
                                ...
                                {players.map((p, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="ml-8 flex-grow flex flex-col justify-center h-full">
                                            ...
                                            <p className="text-[10px] font-bold uppercase tracking-widest mt-1 text-cyan-400 leading-none">{teams.find(t => t.id === p.teamId)?.name}</p>
                                        </div>
                                        <div className="text-right pr-4 flex flex-col justify-center h-full min-w-[120px]">
                                            <span className="text-[10px] font-black uppercase tracking-widest block mb-1 text-cyan-400 opacity-80 leading-none">Assists</span>
                                            <span className="text-5xl font-black italic tracking-tighter text-white leading-[1]">{p.assists}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="grid gap-6">
                    {players.map((p, idx) => (
                        <Link 
                            key={p.id} 
                            to={`/player/${p.id}`}
                            className="bg-slate-900 border border-white/5 rounded-[32px] p-4 md:p-6 flex items-center animate-fadeIn group hover:border-cyan-500 transition-all hover-scale shadow-lg"
                        >
                             <div className="text-2xl md:text-4xl font-black italic text-gray-800 mr-4 md:mr-8 group-hover:text-cyan-500 transition-colors">{getRankBadge(idx)}</div>
                             ...
                             <div className="text-right">
                                <span className="text-3xl md:text-5xl font-black italic text-cyan-400">{p.assists}</span>
