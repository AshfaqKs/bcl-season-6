        ...
        <div className="space-y-12 animate-fadeIn pb-20">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/10 pb-8">
                <div className="text-center md:text-left space-y-2">
                    <p className="text-emerald-400 font-black uppercase tracking-[0.3em] text-xs">Golden Gloves Race</p>
                    <h1 className="text-5xl font-black text-white italic tracking-tighter uppercase leading-none">CLEAN SHEETS</h1>
                </div>
                ...
                <div className="grid gap-6">
                    {players.map((p, idx) => (
                        <Link 
                            key={p.id} 
                            to={`/player/${p.id}`}
                            className="bg-slate-900 border border-white/5 rounded-3xl p-6 flex items-center animate-fadeIn group hover:border-emerald-500 transition-all hover-scale shadow-lg"
                        >
                             <div className="text-4xl font-black italic text-gray-800 mr-8 group-hover:text-emerald-500 transition-colors">{getRankBadge(idx)}</div>
                             ...
                             <div className="text-right">
                                <span className="text-5xl font-black italic text-emerald-400">{p.cleanSheets}</span>
                                <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Clean Sheets</p>
                             </div>
                        </Link>
                    ))}
                </div>
