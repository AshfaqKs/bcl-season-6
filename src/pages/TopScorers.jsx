        ...
        <div className="space-y-12 animate-fadeIn pb-20">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/10 pb-8">
                <div className="text-center md:text-left space-y-2">
                    <p className="text-blue-500 font-black uppercase tracking-[0.3em] text-xs">Golden Boot Race</p>
                    <h1 className="text-5xl font-black text-white italic tracking-tighter uppercase leading-none">TOP SCORERS</h1>
                </div>
                ...
                <div className="grid gap-6">
                    {players.map((p, idx) => (
                        <Link 
                            key={p.id} 
                            to={`/player/${p.id}`}
                            className="bg-slate-900 border border-white/5 rounded-[32px] p-6 flex items-center animate-fadeIn group hover:border-blue-500 transition-all hover-scale shadow-lg"
                        >
                             <div className="text-4xl font-black italic text-gray-800 mr-8 group-hover:text-blue-500 transition-colors">{getRankBadge(idx)}</div>
                             ...
                             <div className="text-right">
                                <span className="text-5xl font-black italic text-blue-500">{p.goals}</span>
                                <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Goals</p>
                             </div>
                        </Link>
                    ))}
                </div>
